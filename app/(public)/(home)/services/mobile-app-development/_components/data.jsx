import { Lightbulb, Smartphone, Zap, CheckCircle } from "lucide-react"

export const mobileAppTypes = [
    {
        title: "Android App Development",
        description: "We create powerful, scalable, and reliable Android apps that reach millions of users. Our expert developers focus on functionality, speed, and intuitive user experience, ensuring apps perform seamlessly across devices. Recognized as a trusted Android application development company in Delhi-NCR, we deliver tailored solutions that combine creativity, strategy, and technology for measurable business growth.",
        icon: "🤖",
        image: "/App Development/Android Development.jpg"
    },
    {
        title: "iOS App Development",
        description: "Our iOS app development services deliver sleek, secure, and high-performing applications tailored for Apple users. We design responsive, intuitive interfaces that meet Apple's strict performance and design standards. From concept to deployment, every app is crafted with precision and innovation, making us a reliable iPhone app development company that helps businesses engage, retain, and delight audiences effectively.",
        icon: "🍎",
        image: "/App Development/ios app development.jpg"
    },
    {
        title: "Cross-Platform App Solutions",
        description: "Expand your reach cost-effectively with high-performing apps for both Android and iOS. Using cutting-edge frameworks like React Native and Flutter, we build scalable cross-platform applications without sacrificing speed, design quality, or user experience. Our solutions maximize impact while reducing costs, enabling businesses to engage diverse audiences seamlessly through a single, cohesive, and future-ready mobile platform.",
        icon: "🔄",
        image: "/App Development/Cross platform app.jpg"
    },
    {
        title: "Custom Mobile App Design & Development",
        description: "Every business is unique, and so are our mobile app solutions. We design and develop custom applications from the ground up, aligning features, workflows, and user journeys with business goals. With a focus on intuitive design, strong functionality, and performance, our apps solve real challenges while delivering engaging experiences tailored to each client's industry and audience.",
        icon: "🎨",
        image: "/App Development/Custom app development.jpg"
    },
    {
        title: "Enterprise & Productivity Apps",
        description: "Boost efficiency with enterprise mobility solutions, internal communication platforms, and productivity apps. Designed for businesses of all sizes, these apps streamline workflows, enhance collaboration, and empower employees. Whether it's project management, CRM, or internal operations, we build secure, scalable solutions that optimize business performance and drive measurable growth across industries.",
        icon: "💼",
        image: "/App Development/Enterprise App.jpg"
    },
    {
        title: "Ecommerce & On-Demand Apps",
        description: "We create engaging ecommerce apps, marketplaces, and on-demand platforms for food delivery, ridesharing, and service booking. With secure payments, catalogue management, and real-time updates, our solutions deliver convenience to customers while boosting revenue. Designed for scalability and user delight, these apps provide seamless shopping and service experiences tailored to your business model.",
        icon: "🛒",
        image: "/App Development/ecommerce app.jpg"
    },
    {
        title: "Education, Healthcare & Fitness Apps",
        description: "From interactive learning platforms and virtual classrooms to telemedicine, appointment booking, and fitness tracking apps, we develop solutions that improve engagement and accessibility. Secure, user-friendly, and data-driven, these apps empower industries to connect with audiences meaningfully enhancing learning, promoting wellness, and simplifying healthcare experiences for students, patients, professionals, and fitness enthusiasts alike.",
        icon: "🏥",
        image: "/App Development/Healthcare app.jpg"
    },
    {
        title: "Fintech, Gaming & Media Apps",
        description: "We design robust apps for financial services, entertainment, and media. From secure payment gateways and digital wallets to streaming, gaming, and content platforms, our solutions balance performance with compliance and creativity. Whether boosting engagement or enabling transactions, we ensure these apps deliver seamless experiences that keep users entertained, informed, and connected globally.",
        icon: "🎮",
        image: "/App Development/Gaming App.jpg"
    }
]

export const faqs = [
    {
        question: "What is your mobile app development process?",
        answer: "Our process involves: idea validation, UI/UX design, app development (iOS, Android, or cross-platform), rigorous testing, and deployment. We maintain clear communication throughout each phase."
    },
    {
        question: "Do you conduct market research or competitor analysis before starting development?",
        answer: "Yes, we can conduct thorough market research and competitor analysis to help refine your app concept and ensure it meets market demands."
    },
    {
        question: "Is it possible to integrate third-party APIs and services into my app?",
        answer: "Yes, we have experience integrating various APIs and services, such as payment gateways, social media platforms, and location services."
    },
    {
        question: "Do you offer UI/UX design services?",
        answer: "Absolutely! We focus on user-centered design, delivering intuitive and visually engaging interfaces to elevate the user experience."
    },
    {
        question: "How much does mobile app development cost?",
        answer: "App development costs depend on factors like features, platform, and complexity. Our quotes are tailored to suit your unique needs and project specifications."
    },
    {
        question: "Do you offer fixed-price or hourly rate options?",
        answer: "We offer both fixed-price and hourly rate options, depending on the project's scope and client preferences."
    },
    {
        question: "How do you handle app maintenance and updates after launch, and do you assist with app store submission?",
        answer: "We offer maintenance and support packages to keep your app secure and updated. We assist with app store submissions and provide support for updates, bug fixes, and feature enhancements."
    },
    {
        question: "What do I need to do and provide to start my app project and get an estimate?",
        answer: "To begin, simply contact us for a free consultation. We'll discuss your app idea and requirements. To provide you with an accurate estimate and a tailored proposal, please share details about your app's features, target audience, platform (iOS, Android, or both), and any specific requirements you have."
    }
]

export const processSteps = [
    {
        step: "01",
        title: "Research & Planning",
        description: "We begin by validating your idea with thorough business analysis, competitor insights, and user research. This stage sets the foundation for designing a product that solves real problems, meets market expectations, and creates measurable impact for your target audience and long-term business growth.",
        icon: Lightbulb,
        color: "from-blue-500 to-purple-600",
        bgColor: "bg-blue-50",
        features: [
            "Idea Validation & Discovery",
            "Market & Competitor Analysis",
            "User Journey Mapping"
        ]
    },
    {
        step: "02", 
        title: "Design & Development",
        description: "Here's where vision meets execution. Our UI/UX designers and developers collaborate to create intuitive, engaging, and robust apps. Every layout, feature, and integration is crafted for performance, scalability, and user delight—ensuring your mobile app stands out in competitive markets while delivering seamless functionality and memorable digital experiences.",
        icon: Smartphone,
        color: "from-purple-500 to-pink-600",
        bgColor: "bg-purple-50",
        features: [
            "UI/UX Wireframing & Prototyping",
            "Agile Development Approach",
            "Third-Party API Integrations"
        ]
    },
    {
        step: "03",
        title: "Testing & Deployment", 
        description: "We ensure flawless functionality with rigorous testing before launch. From security to scalability, every app is refined, optimized, and perfected for the best possible user experience. Once fully tested, we handle app store deployment efficiently, ensuring compliance, smooth approvals, and successful publishing across iOS and Android platforms for maximum reach.",
        icon: CheckCircle,
        color: "from-pink-500 to-red-600",
        bgColor: "bg-pink-50",
        features: [
            "Functional & Security Testing",
            "Performance Optimization",
            "App Store Deployment"
        ]
    },
    {
        step: "04",
        title: "Support & Growth", 
        description: "Post-launch, we provide ongoing maintenance, updates, and analytics to ensure your app evolves with user needs and industry trends. Our continuous support helps improve functionality, resolve issues, and scale features. By tracking performance closely, we keep your app secure, optimized, and positioned for sustainable growth in competitive digital environments.",
        icon: Zap,
        color: "from-green-500 to-blue-600",
        bgColor: "bg-green-50",
        features: [
            "Regular Updates & Enhancements",
            "Bug Fixes & Optimization",
            "Analytics & Performance Tracking"
        ]
    }
]

export const whyChooseUs = [
    "Award-winning UI/UX design expertise",
    "End-to-end app development solutions",
    "Transparent communication & agile methodology",
    "Industry experience across retail, healthcare, education & fintech",
    "Timely delivery with global reach",
    "Affordable packages for startups & enterprises",
    "Scalable apps designed for future growth",
    "Dedicated support beyond launch"
]
